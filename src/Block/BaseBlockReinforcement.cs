...
{
    public abstract class BaseBlockReinforcement : Block
    {
        ...
        public override bool TryPlaceBlock(IWorldAccessor world, IPlayer byPlayer, ItemStack itemstack, BlockSelection blockSel, ref string failureCode)
        {
            ...
            if (result)
            {
                ...
                if (blockEntity != null && blockEntity is BaseBlockEntityReinforcement)
                {
                    ((BaseBlockEntityReinforcement)blockEntity).RecalculateModel(world.BlockAccessor, blockPos);
                }
            }
            return result;
        }
        ...
        public override void OnNeighbourBlockChange(IWorldAccessor world, BlockPos pos, BlockPos neibpos)
        {
            ...
            if (blockEntity != null && blockEntity is BaseBlockEntityReinforcement)
            {
                bool change = ((BaseBlockEntityReinforcement)blockEntity).RecalculateModel(world.BlockAccessor, pos);
                if (change)
                {
        ...
        {
            BaseBlockEntityReinforcement blockEntity = blockAccessor.GetBlockEntity(pos) as BaseBlockEntityReinforcement;
            if (blockEntity == null)
            {
                ...
                if (entityClass != null && entityClass.Length > 0)
                {
                    ...
                    blockEntity = blockAccessor.GetBlockEntity(pos) as BaseBlockEntityReinforcement;
                }
                else
                {
